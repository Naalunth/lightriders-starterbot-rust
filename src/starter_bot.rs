...
impl StarterBot {
	pub fn new() -> Self {
		StarterBot {
			rng_state: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
			board: None,
			id: 0,
			round: 0
		}
...
impl Bot for StarterBot {
	fn get_move(&mut self) -> Move {
		...
		let moves: Vec<Move> = self.board
			.as_ref()
			.iter()
			.flat_map(|b| b.legal_moves(self.id))
			.collect();
		if moves.len() > 0 {
			moves.get(self.rng_state as usize % moves.len()).map(|m| (*m).clone()).unwrap()
		} else {
			Move::Up
		}
	}
